...
from molten_mail.mail import Message, Mail, force_text, sanitize_address
from molten_mail.exceptions import MailUnicodeDecodeError, BadHeaderError
...
def test_message_init():
    msg = Message(subject="subject", recipients=["fake@example.com"], body="body")
...
def test_empty_cc_list():
    msg = Message(subject="subject", recipients=["fake@example.com"])
    assert msg.cc == []
...
def test_sender_as_tuple():
    msg = Message(subject="test", sender=("Me", "me@example.com"))
...
def test_plain_message_with_unicode_attachment():
    ...
    msg.attach(
        data=b"this is a test", content_type="text/plain", filename="ünicöde ←→ ✓.txt"
    )
...
def test_plain_message_with_ascii_converted_attachment():
    ...
    msg.attach(
        data=b"this is a test", content_type="text/plain", filename="ünicödeß ←.→ ✓.txt"
    )
...
def test_unicode_headers():
    ...
    h2 = email.header.Header(
        "From: %s" % sanitize_address("ÄÜÖ → ✓ <from@example.com>")
    )
...
def test_unicode_subject():
    msg = Message(
        subject="sübject", sender="from@example.com", recipients=["to@example.com"]
    )
...
def test_message_charset():
    ...
    msg.html = None
    msg.attach(
        data=b"foobar", content_type="text/csv", headers={"X-Extra-Header": "Yes"}
    )
    assert 'Content-Type: text/plain; charset="utf-8"' in msg.as_string()
